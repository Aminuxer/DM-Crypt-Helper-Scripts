...
* Что случится, если я запущу команду `$ sudo /opt/_dmc.sh /var/tmp/fs1.bin` - только с путём к уже существующему файлу ?
  - Скрипт постарается определить текущее состояние и выполнит предполагаемое действие. Подкюченный контейнер будет отключён, отключенный - попробует подключить через запрос парольной фразы.
...
* Что за метод make_loops ?
  - Этот метод для некоторых старых Live-системы, где петельные блочные устройства не создавались при загрузке.
Использует утилиту mknod. Может быть полезен, если вы подключаете очень много контейнеров.

* Скрипт может испортить мой обычный файл, если я попробую его подкючить ? например _dmc.sh dsc0001.jpg ?
...
* Можно монтировать несколько разных копий одного и того же контейнера ?
  - Плохая идея; Скрипт попробует это определить для предотвращения опасных ошибок.
  Если очень надо, переименуйте старую копию контейнера и подключите её в другой путь. Сделайте резервные копии ДО начала работ.
  С версии 2021-10-26 скрипт преодтвращает монтирование одного и того же файла в несколько loop-устройств.
...
* Можно ли обновлять скрипт до новой версии ?
  - Да. Сделайте бэкап и скачайте скрипт поверх старого. Если не менялся метод шифрования - всё должно работать. В ином случае, используйте четвёртый парамтер (CIPHER) для указания типа шифра.
